...
for y in range(len(Test_years)):
    T_data.append(xr.open_mfdataset('/pl/active/ATOC_SynopticMet/data/ar_data/Research2/3hrly_merra2_hemisphere/'+str(Test_years[y])+'*', chunks = 'auto').sel(lat = slice(-90,-40)).resample(time = '1d').mean()['T'])
...
for t in range(len(TP_id)):
    ...
    latN = -40               # northernmost latitude in domain, must be between 90º and -90º
...
out_folder = '/pl/active/ATOC_SynopticMet/data/ar_data/Research3/CNN_retry/CNN_retry_2V/Contribution_H_L/highH_'
